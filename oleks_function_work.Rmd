...
```{r}
set.seed(20)
k <- st_distance(pdx_sf)
km <- as.matrix(k)
diag(km) <- NA
kmin <- apply(km, 1, min, na.rm=TRUE)

#couldn't find the area/density
a <- st_area(pdx_sf$geometry)
units::set_units(a, km^2)
as.numeric(a)
  
#What we did in class
distance <- seq(1, 30000, 100)
Kd <- sapply(distance, function(x) sum(kmin < x)) # takes a while
