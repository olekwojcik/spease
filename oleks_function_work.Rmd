...
#distance
xy <- as.data.frame(st_coordinates(pdx_sf))
d <- vector()
d[1] <- 0
for(i in 2:nrow(xy)){
lonlat_dist <- distance(lat1=xy$Y[i-1],lon1=xy$X[i-1],lat2=xy$Y[i],lon2=xy$X[i])
d[i] <- as.numeric(lonlat_dist[5]) # the fifth element of the output frame is the distance between the points.
}
